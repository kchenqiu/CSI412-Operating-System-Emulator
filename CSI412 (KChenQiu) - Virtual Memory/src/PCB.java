...
public class PCB {
	...
	private VirtualToPhysicalMapping[] pageArray = new VirtualToPhysicalMapping[100];
	private String name;
	...
	public void AllocateMemory(int size) {
		int amountAllocated = size;
		for(int i = 0; i < amountAllocated; i++) {
			if(pageArray[i] == null) {
				pageArray[i] = new VirtualToPhysicalMapping();
			}
			else {
				amountAllocated++;
			}
	...
	public void newPage(int target) {
		pageArray[target]= new VirtualToPhysicalMapping();
	}
	
	public void setPage(int target, int address) {
		pageArray[target].physicalPageNumber = address;
	}
	
	public void ResetTargetPage(int target) {
		pageArray[target] = null;
	}
	
	public VirtualToPhysicalMapping getPage(int i) {
		return pageArray[i];
	}
