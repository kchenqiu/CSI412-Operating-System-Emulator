...
public class Scheduler {
		...
		if(current == null) {
			...
			if(!normalPriorityProcess.isEmpty() && current == normalPriorityProcess.getFirst()) {
				...
				if(current.isDone()) {
					...
					for(int i = 0; i < 100; i++) {
						if(current.getPage(i) != null && current.getPage(i).physicalPageNumber != -1) {
							OS.FreeMemory(current.getPage(i).physicalPageNumber, 1024);
						}
					}
				}
				else {
	...
	public PCB getRandomProcess() {
		...
		if(rand == 0) {
			if(highPriorityProcess.size() > 0) {
				rand = random.nextInt(highPriorityProcess.size());
				return highPriorityProcess.get(rand);
			}
			else if(lowPriorityProcess.size() > 0){
				rand = random.nextInt(lowPriorityProcess.size());
				return lowPriorityProcess.get(rand);
			}
			else {
				rand = random.nextInt(normalPriorityProcess.size());
				return normalPriorityProcess.get(rand);
			}
		}
		else if(rand == 1) {
			if(normalPriorityProcess.size() > 0) {
				rand = random.nextInt(normalPriorityProcess.size());
				return normalPriorityProcess.get(rand);
			}
			else if(lowPriorityProcess.size() > 0){
			...
			if(lowPriorityProcess.size() > 0){
				rand = random.nextInt(lowPriorityProcess.size());
				return lowPriorityProcess.get(rand);
			}
			else if(normalPriorityProcess.size() > 0) {
				rand = random.nextInt(normalPriorityProcess.size());
				return normalPriorityProcess.get(rand);
			}
			else {
