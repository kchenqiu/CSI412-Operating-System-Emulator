	...
	public void run() {	
		...
		while(true) {		
			...
			case AllocateMemory:
				...
				scheduler.getCurrentlyRunning().AllocateMemory(size);
				
				//finds a spot for the allocation
				...
				//allocates memory if there is a spot, do nothing if there isn't
				if(memoryLocation != -1) {
					memoryLocation = memoryLocation - size + 1;
					
					for(int i = memoryLocation; i < memoryLocation + size; i++) {
						memoryMap[i] = true;
						scheduler.getCurrentlyRunning().setPage(i - memoryLocation, i);
					}
				}
			...
			case FreeMemory:
				...
				for(int i = pointer; i < (size + pointer); i++) {
					if(scheduler.getCurrentlyRunning().getPage(i-pointer).physicalPageNumber != -1) {
						scheduler.getCurrentlyRunning().ResetTargetPage(i-pointer);
					}
					memoryMap[i] = false;
				}
